...
const { resolveKeyGenParams, generateVaultKey } = require('../keygen');
...
test('kek: embed/extract roundtrip with every KEK type', async function() {
	...
	for (const kekJwk of [ makeEcKekJwk('ec-kek'), makeRsaKekJwk('rsa-kek'),
		...
		const { embeddingKeyId, embeddedKey } = mgr.embed(key.secretKey,
		...
		const extracted = mgr.extract(embeddingKeyId, embeddedKey, key.kid);
...
test('kek: extract failure modes', async function() {
	...
	const { embeddingKeyId, embeddedKey } = mgr.embed(key.secretKey, { kid: key.kid, iat: 1000 });
	// Unknown KEK id.
	assert.throws(function() { mgr.extract('other-kek', embeddedKey, key.kid); },
				  KekUnavailableError);
	// kid mismatch between row and embedded claims.
	assert.throws(function() { mgr.extract(embeddingKeyId, embeddedKey, 'ffffffff-ffff-ffff-ffff-ffffffffffff'); },
				  /kid mismatch/);
	// Tampered embedded JWE fails to decrypt.
	const tampered = embeddedKey.slice(0, -4) + 'AAAA';
	assert.throws(function() { mgr.extract(embeddingKeyId, tampered, key.kid); });
});
...
test('kek: retired KEK unwraps old embeddings', async function() {
	...
	const wrapped = oldMgr.embed(key.secretKey, { kid: key.kid, iat: 1 });
	...
	assert.deepEqual(newMgr.extract(wrapped.embeddingKeyId, wrapped.embeddedKey, key.kid),
					 key.secretKey);
});
